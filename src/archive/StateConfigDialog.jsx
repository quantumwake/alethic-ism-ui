...
const configOptions = [
    ...
    {config_type: 'StateConfigAudio', config_name: 'Audio'},
    {config_type: 'StateConfigUserInput', config_name: 'User Input (Interaction)'},
];
...
function StateConfigDialog({ isOpen, setIsOpen, nodeId }) {
    ...
    const onChangeDropDownSelection = (type, value) => {
        ...
        switch (type) {
            ...
            case "template":
                ...
                break
            case "filter":
                nodeData.config.filter_id = value
                break
    ...
    return (<>
        <div className="mt-5 p-4 border-2 border-gray-200 rounded-lg shadow-sm">
            ...
            <table className="min-w-full divide-y divide-gray-300">
                <tbody className="bg-white divide-y divide-gray-200">
                {/*{(nodeData?.state_type === "StateConfigUserInput") && (<>*/}
                ...
                {(nodeData?.state_type === "StateConfigStream" ||
                    ...
                    nodeData?.state_type === "StateConfigAudio" ||
                    nodeData?.state_type === "StateConfigUserInput" ||
        ...
        {/*Advanced Settings Section Start Here*/}
        <div
            className="my-2 flex justify-between items-center cursor-pointer"
            onClick={() => setIsAdvancedCollapsed(!isAdvancedCollapsed)}>
            <label className="font-semibold">Advanced Settings</label>
            <button
                className="text-sm text-blue-700 hover:text-gray-700 focus:outline-none">
                {isAdvancedCollapsed ? 'Show' : 'Hide'}
            </button>

        </div>

        <div className="mt-5 p-4 border-2 border-gray-200 rounded-lg shadow-sm">
            <ActionDefinition nodeId={nodeId}/>
        </div>
        ...
        {!isAdvancedCollapsed && (<>
                ...
                <table className="w-full border-collapse">
                    ...
                    {flags.map((flag, index) => (
                        <React.Fragment key={flag}>
                            {index % 2 === 0 &&
                                <tr/>} {/* Start a new row for every two items */}
                            <td className="px-3 py-3 border">
                                <div className="flex w-max flex-row items-center">
                                    <div className="flex-1 w-1/2">
                                        <label
                                            className="mr-4">{flag.replace('flag_', '').replace(/_/g, ' ')}</label>
                                    </div>
                                    <div className="flex-1 w-1/2">
                                        <CustomSwitch
            ...
            {(nodeData?.state_type === "StateConfig" ||
                ...
                <div className="mt-5 p-4 border-2 border-gray-200 rounded-lg shadow-sm">
                    ...
                    <p>Columns within the state are dynamically defined during the processing of
                        state
                        output. Additionally, you can define columns that are not part of the
                        state
                        output, such as:
                    </p>
                    ...
                    <ul>
                        <li>
                            <strong>Evaluated Columns:</strong> Columns whose values are
                            calculated
                            based on other data.
                        </li>
                        <li><strong>Constant Value Columns:</strong> Columns with fixed values.
                        </li>
                    </ul>
                    ...
                    <ul>
                        <li>
                            ...
                            processor's data will take precedence and overwrite the constant
                            value.
                        </li>
                        <li>
                            If a column is defined without a value, it is assumed that the value
                            will be
                            provided at the time of state data creation by the connected state
                            output to
                            the processor.
                        </li>
                    </ul>
                ...
                <div className="mt-5 p-2 border-2 border-gray-100">
                    <div className="mt-2">
                        <label className="pr-2 font-medium">Definition of Primary Key
                            Columns</label>
                    ...
                    <div className="mt-2">
                        <label className="pr-2 font-medium">Definition of Inherited
                            Columns</label>
                        <StateConfigDataKeyDefinitionHandler nodeId={nodeId}
                    ...
                    <div className="mt-2">
                        <label className="pr-2 font-medium">Definition of Remap Query State
                            Columns</label>
                        <StateConfigDataKeyDefinitionHandler nodeId={nodeId}
